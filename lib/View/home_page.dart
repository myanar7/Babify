...
class _HomePageState extends ConsumerState<HomePage> {
  ...
  Container setValue(Activity timerActivity) {
    if (timerActivity is SleepActivity) {
      ...
      return printTimerActivity(timerActivity, a.finishTime, a.second, path, "BreastFeed");
    } else if (timerActivity is BottleMilkActivity) {
      BottleMilkActivity a = timerActivity;
      String path = "assets/icons/milk.png";
      return printChoiceActivity(timerActivity, a.type, a.amount, path, "Bottle");
      ...
      return printHealthActivity(timerActivity, a.name, a.dose, a.type, path, "Medication");
    }else if (timerActivity is VaccinationActivity) {
      VaccinationActivity a = timerActivity;
      String path = "assets/icons/vaccination.png";
      return printHealthActivity(timerActivity, a.name, "", "", path, "Vaccination");
    } else if (timerActivity is MeasureActivity) {
      MeasureActivity a = timerActivity;
      String path = "assets/icons/measure.png";
      return printHealthActivity(timerActivity, a.weight, a.height, a.head, path, "Measure");
    }
    return Container();
  }

  Container printTimerActivity(Activity a, DateTime finishTime, int second, String path, String info) {
            ...
            Expanded(
              child: Column(
                children: [
                  ...
                  Expanded(
                    child: Text(a.startTime.hour.toString() +
                        ":" +
                        startMinute +
                        " to " +
                        finishTime.hour.toString() +
                        ":" +
                        finishMinute, style: const TextStyle(fontWeight: FontWeight.w200),),
                  ),
                  Expanded(
                    child: Text(a.note, style: const TextStyle(fontWeight: FontWeight.w200),),
                  ),
                ],
              ),
              flex: 1,
  ...
  Container printChoiceActivity(Activity a, String type, String amount, String path, String info) {
      var startMinute = a.startTime.minute.toString();
      if (a.startTime.minute < 10) {
        startMinute = "0" + a.startTime.minute.toString();
      }
          ...
          children: [
            ...
            Expanded(
              child: Column(
                children: [
                  Expanded(
                    child: Text(info != 'Diaper' ? info + " (" + type + "):" + " " + amount + " ml" : info + ": " + type),
                  ),
                  Expanded(
                    child: Text(a.startTime.hour.toString() +
                        ":" +
                        startMinute, style: const TextStyle(fontWeight: FontWeight.w200),),
                  ),
                  Expanded(
                    child: Text(a.note, style: const TextStyle(fontWeight: FontWeight.w200),),
                  ),
  ...
  Container printHealthActivity(Activity a, String input1, String input2, String input3, String path, String info) {
      var startMinute = a.startTime.minute.toString();
      if (a.startTime.minute < 10) {
        startMinute = "0" + a.startTime.minute.toString();
      }
            ...
            Expanded(
              child: Column(
                children: [
                  Expanded(
                    child: Center(
                      child: Text(info == 'Medication' ? info + ": " + input1 + ": " + input2 + input3: info=='Vaccination' ? 
                      info + ": " + input1 :  "Weight:" + input1 +  "kg Height:"  + input2 + "cm Head:" + input3 + "cm",
                      ),
                    ),flex: 1,
                  ),
                  Expanded(
