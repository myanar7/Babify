    ...
    for(int i=0; i<state.length; i++){
      if(state[i].id == id){
        if(state[i] is SleepActivity){
          timerActivity = SleepActivity(id: id, startTime: startTime, finishTime: finishTime, second: second, note: note);
        }else{
          timerActivity = TummyActivity(id: id, startTime: startTime, finishTime: finishTime, second: second, note: note);
        }
        remove(state[i]);
